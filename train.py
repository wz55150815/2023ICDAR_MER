...
if params['finetune'] and (checkpoints_root_path / "model.pkl").exists():
    print('loading pretrain model weight')
    # new_model_save_path = sorted(checkpoints_root_path.glob('*'))[-1]
    # load_checkpoint(model,
    #                 optimizer,
    #                 new_model_save_path / "optim.pkl",
    #                 new_model_save_path / "model.pkl")
    model.load_state_dict(torch.load(checkpoints_root_path / "model.pkl"))
    optimizer.load_state_dict(torch.load(checkpoints_root_path / "optim.pkl"))
