...
def updata_lr(optimizer, current_epoch, current_step, steps, epoches, initial_lr):
    if current_epoch < 1:
        ...
        new_lr = 0.5 * (1 + math.cos(
            (current_step + 1 + (current_epoch - 1) * steps) * math.pi / (epoches * steps))) * initial_lr

    for param_group in optimizer.param_groups:
        param_group['lr'] = new_lr


def save_checkpoint(model, optimizer, word_score, struct_score, ExpRate_score, epoch, optimizer_save=False,
                    path='checkpoints'):
    ...
    torch.save(state["model"], filename / "model.pkl")
    if optimizer_save:
        torch.save(state["optimizer"], filename / "optim.pkl")
...
def cal_score(probs, labels, mask):
    ...
    struct_label = rearrange(struct_label, "b l e->b (l e)")
    line_right = 0
    word_pred = word_probs.max(-1)[-1]

    struct_mask = mask[:, :, 1]
    struct_probs = struct_probs * struct_mask[:, :, None]
    # struct_probs = struct_probs.contiguous().view(batch_size, -1)
    struct_probs = rearrange(struct_probs, "b l e->b (l e)")
    ...
    struct_scores = [SequenceMatcher(None, s1[:int(np.sum(s3))], s2[:int(np.sum(s3))], autojunk=False).ratio() * (
            len(s1[:int(np.sum(s3))]) + len(s2[:int(np.sum(s3))])) / len(s1[:int(np.sum(s3))]) / 2
                     for s1, s2, s3 in zip(struct_label.cpu().detach().numpy(), struct_pred.cpu().detach().numpy(),
                                           mask.cpu().detach().numpy())]
